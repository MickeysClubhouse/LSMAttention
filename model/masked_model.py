...
class MaskedLinear(nn.Linear):
    ...
    def __init__(self, in_features, out_features, bias=True):
        ...
        self.register_buffer('mask', torch.ones(out_features, in_features))  # 初始化全1

        self.masked_weight = None

    def set_mask(self, mask):  # 手动设置mask
...
class EasyMADE(nn.Module):
    ...
    def __init__(
        ...
        if not residual_connections:
            self.net.pop()
        self.net = nn.Sequential(*self.net)  # 组合到一个sequential里
    ...
    def update_masks(self, invoke_order=None):
        ...
        if self.nout > self.nin:
            ...
            # 例如: 输入 [aa] [b] [cc] 则需要把三个mask:[m1][m2][m3]扩展成5位:[m1][m1][m2][m3][m3]
        ...
        layers[0].set_mask(masks[0]) #只需要在第一层加mask
    ...
    def forward(self, x):
        ...
        # update mask
        update_mask = (x == 0).any().item()

        # Update mask if necessary
        if update_mask:
            # Call set_mask method to update the first layer's mask
            # You need to specify the appropriate mask based on your conditions
            # For example, you can create a mask with 0s at specific positions
            # and 1s elsewhere.
            new_mask = torch.ones_like(self.net[0].mask)  # Initialize with all 1s
            # Modify new_mask according to your criteria
            # For example, if the first element of x is 0, set the corresponding
            # elements in new_mask to 0
            for i in range(x[0].shape[0]):
                if x[0, i] == 0:  # 需要增加mask
                    self.net[0].mask[:,i] = 0

            self.net[0].set_mask(new_mask)
...
if __name__ == '__main__':
    ...
    # (nin, hiddens, nout, input_bins, direct_io) == (input num, hidden dims, output dim,
    ...
    for nin, hiddens, nout, input_bins, direct_io in configs_with_input_bins:
        ...
        print(model)
        for k in range(nout):
            inp = torch.tensor((rng.rand(1, nin)*[1,0,1]).astype(np.float32),
                               requires_grad=True)
            loss = model(inp)
            l = loss[0, k]
            l.backward()
